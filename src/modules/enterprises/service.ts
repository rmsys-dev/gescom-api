...
import { enterpriseLogoService } from "./logo/service.js";
...
export class EnterprisesService {
  ...
  public async listForAuthenticatedUser(
    ...
    const enterpriseIds = page.map((row) => row.enterprise!.id);
    const [parametersByEnterprise, hasLogoByEnterprise] = await Promise.all([
      resolveEnterpriseParametersMany(enterpriseIds),
      enterpriseLogoService.hasLogoMany(enterpriseIds),
    ]);
    ...
    return {
      items: mapMembershipsToListItem(
        page.map((row) => ({
          ...
          hasLogo: hasLogoByEnterprise.get(row.enterprise!.id) ?? false,
        })),
      ),
      total,
      limit,
  ...
  public async getById(id: string) {
    ...
    const [parameters, hasLogo] = await Promise.all([
      resolveEnterpriseParameters(id),
      enterpriseLogoService.hasLogo(id),
    ]);
    return {
      ...enterprise,
