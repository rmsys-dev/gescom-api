...
const BODY_EXAMPLES = {
  ...
  convertBudgetToOsSchema: {
    items: [{ budgetItemId: "{{budgetItemId}}", quantity: 1 }],
  },
  convertOsToSaleSchema: {
    status: "ABERTA",
    items: [{ workOrderItemId: "{{workOrderItemId}}", quantity: 1 }],
  },
  createProductWithEnterpriseSchema: {
    product: { description: "FILTRO DE OLEO" },
    enterprise: {
  ...
  patchProductSchema: { description: "FILTRO DE OLEO ATUALIZADO" },
  ...
  createProductsNcmSchema: {
    ncm: "84212300",
    description: "FILTROS DE OLEO",
  },
  createProductsCestSchema: {
    cest: "0100100",
    description: "FILTROS DE OLEO",
    productsNcmId: "{{productsNcmId}}",
  },
  createProductsAnpSchema: {
    anp: "210203001",
    description: "GASOLINA C COMUM",
  },
  createProductsNbsSchema: {
    lc116Item: "14.01",
    lc116Description: "Lubrificacao, limpeza, etc.",
    nbs: "1.1502.10.00",
    ...
    indop: "100101",
    cClassTrib: "000001",
    cClassTribName: "Tributacao integral",
  },
  ...
  createPisCofinsSituationSchema: {
    cst: "01",
    description: "OPERACAO TRIBUTAVEL",
    type: "SAIDA",
    framing: 1,
  },
  createProductTaxationSchema: {
    cstPisEntradaId: "{{cstPisEntradaId}}",
    cstPisSaidaId: "{{cstPisSaidaId}}",
    cstCofinsEntradaId: "{{cstCofinsEntradaId}}",
    cstCofinsSaidaId: "{{cstCofinsSaidaId}}",
    productsEnterprisesId: "{{productEnterpriseId}}",
    icmsTaxationId: "{{icmsTaxationId}}",
  },
  createProductGroupSchema: { description: "Filtros" },
  createProductSubgroupSchema: { description: "Filtros de oleo" },
  createProductBrandSchema: { description: "Mann Filter" },
  createTypeNetworkSchema: { description: "WHATSAPP" },
  createTypeSupplierCustomerSchema: { description: "CONSUMIDOR FINAL" },
  createPaymentTypeSchema: {
    description: "DINHEIRO",
    paymentType: "A_VISTA",
  },
  createStockSectorSchema: { description: "DEPOSITO PRINCIPAL" },
  createStockLocationSchema: { stockSectorId: "{{stockSectorId}}" },
  ...
  createVehicleSchema: {
    ...
    vehicleYear: 2020,
    renavam: "12345678901",
    licensingStateId: "{{stateId}}",
    entireCode: "VEIC-001",
    rntrcCode: "12345678",
    vehicleType: "UTILITARIO",
    bodyType: "NAO_APLICAVEL",
  ...
  createVehiclesEnterprisesMemberSchema: {
    ...
    enterprisesMembersId: "{{memberId}}",
    status: "ATIVO",
  },
  patchVehiclesEnterprisesMemberSchema: { status: "INATIVO" },
  createMechanicSalesItemSchema: {
    mechanic: "{{memberId}}",
    salesItemsId: "{{saleItemId}}",
    comissionService: 10,
  },
  patchMechanicSalesItemSchema: { comissionService: 15 },
...
function formatFolderName(key) {
  const names = {
    ...
    vehicles: "Veículos",
    "vehicles-enterprises-members": "Veículos x Membros",
    "mechanic-sales-items": "Comissões de Mecânicos",
  };
...
function createCollection(endpoints) {
  ...
  const collection = {
    ...
    variable: [
      ...
      { key: "budgetItemId", value: "" },
      { key: "workOrderItemId", value: "" },
      { key: "productId", value: "" },
...
function createEnvironment(name, baseUrl) {
  return {
    ...
    values: [
      ...
      { key: "budgetItemId", value: "", type: "default", enabled: true },
      { key: "workOrderItemId", value: "", type: "default", enabled: true },
...
function main() {
  ...
  const expectedFiles = new Set([
    "generate-complete-collection.mjs",
    "generation-summary.json",
    "gescom-api-complete.postman_collection.json",
    "gescom-api-local.postman_environment.json",
    ...modulePaths.map((p) => path.basename(p)),
  ]);
  ...
  for (const entry of fs.readdirSync(OUT_DIR)) {
    ...
    console.log(`✓ Removido órfão: ${entry}`);
  }
